...
public class EditCommandParserTest {
    ...
    @Test
    public void parse_allLeadFieldsSpecified_success() {
        Index targetIndex = INDEX_SECOND_PERSON;
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + EMAIL_DESC_AMY + ADDRESS_DESC_AMY
                + MEETING_TIME_DESC_AMY + NAME_DESC_AMY + KEY_MILESTONE_DESC_BOB + TAG_DESC_HUSBAND + TAG_DESC_FRIEND;

        EditLeadDescriptor descriptor = new EditLeadDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withMeetingTime(VALID_MEETING_TIME_AMY).withTags(VALID_TAG_HUSBAND, VALID_TAG_FRIEND)
                .withKeyMilestone(VALID_KEY_MILESTONE_BOB).build();
        EditLeadCommand expectedCommand = new EditLeadCommand(targetIndex, descriptor);
